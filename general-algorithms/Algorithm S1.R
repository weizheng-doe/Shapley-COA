################################################################
######################code for Algorithm S1#####################
################################################################

# Note:
# This code only describes the framework of Algorithm S1, 
# some of which need to be predefined, 
# such as the model, the sampling function for vairables, the conditional sampling function for variables,
# and the primitive polynomial (f_d) when the COA method is used with a prime power d.



####=================1. Code for the LS method==========================
##--------predefined function1: the model------------
# Input: values of variables (a vector); Output: value of the model
# model<-function(x){}

##--------predefined function2: the sampling function for vairables------------
# Input: a subset of variables; Output: values of these variables (a vector)
# x.sample<-function(set){}

##--------predefined function3: the conditional sampling function for variables------------
# Input: set_sample: variabels that need to be sampled;  x_n: given values of variables; set_given: variables of x_n;
# Output: sampled values of variables in set_sample (a vector).
# condition.sample<-function(set_sample,x_n,set_given){}
...
##--------Main function-----------
# Input: 
# d: the number of players; n: the sample size; 
# model, x.sample, condition.sample: predefined functions
# n0,n1,n2: parameters in the Monte Carlo algorithm

# Output: 
# sh: a vector including Shapley values of all players.

est.shsals<-function(d,n,model,x.sample,condition.sample,n0, n1, n2){
  sh<-rep(0,d); k<-n/d;
  y<-rep(0,n0)
  for(t in 1:n0){
    y[i]<-model(x.sample(c(1:d)))
  }
  vary<-var(y)
...
# Output: 
# sh: a vector including Shapley values of all players.
est.shsacoa<-function(d,n,model,x.sample,condition.sample,n0, n1, n2){
  sh<-rep(0,d); k<-n/d; m<-d*(d-1);
  y<-rep(0,n0)
  for(t in 1:n0){
    y[i]<-model(x.sample(c(1:d)))
  }
...
# Output: 
# sh: a vector including Shapley values of all players.
est.shsacoa<-function(d,p,f_d,n,model,x.sample,condition.sample,n0, n1, n2){
  sh<-rep(0,d); k<-n/d; m<-d*(d-1);
  y<-rep(0,n0)
  for(t in 1:n0){
    y[i]<-model(x.sample(c(1:d)))
